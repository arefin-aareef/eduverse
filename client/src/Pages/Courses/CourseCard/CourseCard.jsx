...
const CourseCard = ({ item }) => {
  ...
  const handleAddToFavorite = item => {
    console.log(item);
    if(user && user.email) {
      const favoriteItem = { courseId: _id, course_name, image, price, email: user.email }
    ...
    <div className="card card-compact  bg-base-100 shadow-xl">
      <figure>
        <div className="relative">
          ...
          />
          <button onClick={() => handleAddToFavorite(item)} className="btn btn-sm btn-outline mt-2 me-2 text-white hover:bg-red-500 absolute top-0 right-0">Add To Favorites <MdOutlineFavoriteBorder /></button>
